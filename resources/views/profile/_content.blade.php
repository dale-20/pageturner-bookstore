...
<div class="container py-5">
    ...
    <div class="row g-4">
                ...
                <div class="card-body text-center p-4">
                    ...
                    <div class="mb-4 position-relative d-inline-block">
                        ...
                        <div id="avatarWrapper"
                            class="rounded-circle overflow-hidden d-inline-flex align-items-center justify-content-center position-relative"
                            ...
                            onclick="document.getElementById('profile_photo').click()" title="Click to change photo">
                            ...
                            @else
                                ...
                                <span id="avatarInitial" class="text-white"
                                    style="font-size: 3.5rem; font-weight: 700;">{{ substr($user->name, 0, 1) }}</span>
                            @endif
                        ...
                        {{-- Online indicator --}}
                        <div class="position-absolute bottom-0 end-0 p-2 bg-success border border-3 border-white rounded-circle"
                    ...
                    @if($user->email_verified_at)
                        <div class="alert alert-success py-2 px-3 mb-4 rounded-pill d-inline-flex align-items-center"
                            style="background: rgba(25, 135, 84, 0.1); border: none; color: #198754;">
                            <i class="bi bi-check-circle-fill me-2"></i>
                            <small>Email verified</small>
                        </div>
                    @else
                        <div class="alert alert-warning py-2 px-3 mb-4 rounded-pill"
                            style="background: rgba(255, 193, 7, 0.1); border: none; color: #856404;">
                            <i class="bi bi-exclamation-triangle-fill me-2"></i>
                            <small>Email not verified</small>
                            <a href="{{ route('verification.notice') }}" class="text-primary fw-bold d-block mt-1">Verify
                                now →</a>
                        </div>
                    @endif
                    ...
                    @if($isAdmin)
                        <div class="d-grid gap-2">
                            <a href="{{ route('admin.dashboard') }}"
                                class="btn btn-outline-danger btn-sm rounded-pill">Dashboard</a>
                            <a href="{{ route('admin.orders', 'pending') }}"
                                class="btn btn-outline-danger btn-sm rounded-pill">Manage Orders</a>
                            <a href="{{ route('admin.books.index') }}"
                                class="btn btn-outline-danger btn-sm rounded-pill">Manage Books</a>
                        </div>
                    @else
                        <div class="d-grid gap-2">
                            <a href="{{ route('orders.index') }}" class="btn btn-outline-danger btn-sm rounded-pill">My
                                Orders</a>
                            <a href="{{ route('books.index') }}" class="btn btn-outline-danger btn-sm rounded-pill">Browse
                                Books</a>
                        </div>
                    @endif
                </div>
        ...
        <div class="col-md-8" style="min-height: 500px">
            <div class="card border-0 shadow-lg"
                ...
                <div class="card-body p-4">
                    ...
                    <form method="POST" action="{{ route('profile.update') }}" id="profileForm"
                        ...
                        <div class="mb-4">
                            <div class="input-group">
                                ...
                                <input type="text"
                                    ...
                                    id="name" name="name" value="{{ old('name', $user->name) }}" placeholder="Full Name"
                        ...
                        <div class="mb-4">
                            <div class="input-group">
                                ...
                                <input type="email"
                                    ...
                                    placeholder="Email Address" style="padding: 1rem; border-color: #e2e8f0;">
                            </div>
                            @error('email')
                                <div class="invalid-feedback d-block mt-2">
                                    <i class="bi bi-exclamation-circle-fill me-1"></i> {{ $message }}
                            ...
                            @if(!$user->email_verified_at)
                                <div class="mt-2 d-flex align-items-center gap-2">
                                    ...
                                    <a href="{{ route('verification.notice') }}" class="text-danger fw-bold small">Resend
                                        verification →</a>
                                </div>
                            @endif
                        </div>
                        ...
                        {{-- Change Password --}}
                        <div class="password-section p-4 rounded-4 mb-4"
                            style="background: linear-gradient(145deg, #f1f5f9, #e9edf2);">
                            ...
                            <div class="password-strength mb-3 px-2">
                                ...
                                <small class="text-muted mt-2 d-block" id="passwordHelp">Use at least 8 characters with
                                    letters & numbers</small>
                            </div>
                        ...
                        <div class="d-flex justify-content-end gap-3">
                            <button type="reset" class="btn btn-outline-secondary px-5 py-3 rounded-pill"
                            ...
                            <button type="submit" class="btn btn-danger px-5 py-3 rounded-pill shadow-lg"
                                ...
                                id="submitBtn">
                                <span class="btn-text">
                                    <i class="bi bi-check-lg me-2"></i>Save Changes<i
            ...
            @if(!$isAdmin)
                <div class="card border-0 shadow-lg mt-4"
                    style="border-radius: 20px; border-left: 5px solid #dc2626 !important;">
                    <div class="card-body p-4">
    ...
    @if(session('status') === 'profile-updated' || session('success'))
        <div id="toastSuccess" class="toast align-items-center border-0 text-white show" role="alert"
            ...
            <div class="toast-progress"
                style="height: 4px; background: rgba(255,255,255,0.4); border-radius: 0 0 16px 16px; overflow: hidden;">
                <div id="toastSuccessBar"
                    style="height: 100%; width: 100%; background: rgba(255,255,255,0.8); transition: width linear;"></div>
            </div>
        </div>
    @endif
    ...
    @if($errors->any())
        <div id="toastError" class="toast align-items-center border-0 text-white show" role="alert"
            ...
            <div class="d-flex align-items-center p-3">
                ...
                <button type="button" class="btn-close btn-close-white ms-3 align-self-start mt-1"
                    data-bs-dismiss="toast"></button>
            </div>
            <div class="toast-progress"
                style="height: 4px; background: rgba(255,255,255,0.4); border-radius: 0 0 16px 16px; overflow: hidden;">
                <div id="toastErrorBar"
                    style="height: 100%; width: 100%; background: rgba(255,255,255,0.8); transition: width linear;"></div>
            </div>
        </div>
    @endif
